<?php
/**
 * Page template
 *
 * Toont:
 * - Header
 * - Hero (via template-part + featured image)
 * - Page content
 * - Footer
 *
 * Alles is 100% PHP (klassieke theme aanpak)
 */

get_header();
mm_render_page_header();
?>

<?php
/**
 * Hero template-part
 * (optioneel, afhankelijk van jouw theme-structuur)
 */
get_template_part('template-parts/hero/hero');
?>

<?php
$show_hero = get_post_meta(get_the_ID(), '_mm_show_hero', true);

if ($show_hero === '1') {
    mm_render_hero();
}
?>

<main>

  <?php
  /**
   * HERO SECTIE
   * Wordt alleen getoond als de pagina een featured image heeft
   */
  if (has_post_thumbnail()) :
  ?>
    <section class="relative h-[60vh] w-full overflow-hidden">

      <!-- Hero afbeelding -->
      <img
        src="<?php echo esc_url(get_the_post_thumbnail_url(null, 'full')); ?>"
        alt="<?php echo esc_attr(get_the_title()); ?>"
        class="absolute inset-0 w-full h-full object-cover"
      >

      <!-- Donkere overlay -->
      <div class="absolute inset-0 bg-black/40"></div>

      <!-- Hero content -->
      <div class="relative z-10 h-full flex items-center justify-center text-center px-6">
        <h1 class="text-white text-4xl md:text-6xl font-bold">
          <?php the_title(); ?>
        </h1>
      </div>

    </section>
  <?php endif; ?>

  <?php
  /**
   * PAGINA CONTENT
   */
  ?>
  <section class="max-w-7xl mx-auto px-6 py-16">

    <?php
    while (have_posts()) :
      the_post();
      the_content();
    endwhile;
    ?>

  </section>

</main>

<?php get_footer(); ?>
